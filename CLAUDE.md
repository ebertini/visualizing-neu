...
- `TopicVizPrototypes/` — **the user's own** topic-model visualization prototypes: `topic_flow.html` (funding over time) and `what_we_can_see.html`, now a multi-section dashboard — the original agency×year abstract-coverage heatmap + mosaic/cliff panels, plus (added in response to direct PI feedback) a facet-grid unit visualization ("Every grant, arranged" — arrange/split/color by any of 9 facets: agency, year, college, NEU attribution, abstract presence/source, parent theme, dollar band, PI-matched), a field-by-field missingness matrix, an abstract-sourcing attrition funnel (main pipeline + the M2 orphan-recovery branch), and a "What we cannot see" negative-space card grid. Reads EnricoVis's canonical BERTopic/SPECTER2 output as a read-only upstream input (`docs/EnricoVis/data/{grants_umap,topics}.json`) via `src/build_viz_aggregates.py`, which now also emits `facets.json`/`missingness.json`/`funnel.json`; writes only into its own `data/` and `shared/` (CSS/JS kit extracted from EnricoVis's house style, kept separate). **House convention: all audience-facing copy (section headers, "How this is computed" drawers) is plain-language only** — no file/column/function names (`.parquet`, `grant_id`, etc.); that detail belongs in this repo's own docs, not the dashboard. **Still open, blocked on the PI:** a topic-reliability/manual-inspection panel (his classification-based topic method isn't documented anywhere yet; he also wants a student from related prior work looped in first) and whether this dashboard gets wired into the GitHub Pages deploy (`docs/onlineoutput/`) or stays local-only. The earlier-planned "Round 2" (money-vs-volume slope chart, treemap, agency→theme→college Sankey) hasn't been picked back up.
