...
- `.rowlabel` is a class name shared across *multiple* panels in `what_we_can_see.html` (the facet grid's row labels AND the Coverage tab's heatmap row labels) — CSS/JS changes targeting one must be scoped (e.g. `#facetlabels .rowlabel{...}`), never applied to the bare class.
