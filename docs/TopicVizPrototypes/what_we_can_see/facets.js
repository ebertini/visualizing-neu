...
//
// Briefly removed in favor of a fixed width shared with the suggestion
// select (both toolbar-restructuring session), then reinstated for the
// search input specifically — PI feedback: wanted the search box flexible
// again (auto-fit to its own text/match-count, like before), just capped by
// a max width rather than a single fixed value. The suggestion select kept
// its fixed width; only the search input calls this again (see grid.js).
